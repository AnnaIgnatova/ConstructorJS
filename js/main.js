...
const createHeader = ({title, header: { logo, social, menu } }) => {
  const headerBlock = getElement('header');
  const container = getElement('div', ['container']);
  const wrapper = getElement('div', ['header']);

  createHeaderLogo(title, logo, wrapper);
  createHeaderNavLinks(menu, container, wrapper);
  createHeaderSocialBlock(social, wrapper);

  headerBlock.append(container);
  container.append(wrapper);

  return headerBlock
};

const clickBurgerMenu = (burgerMenu, header) => {
  burgerMenu.addEventListener('click', () => {
    burgerMenu.classList.toggle('menu-button-active');
    header.classList.toggle('header-active');
  });
}

const createHeaderLogo = (title, logo, wrapper) => {
  if (logo) {
...
const createHeaderNavLinks = (menu, container, wrapper) => {
  if (menu) {
    const menuNav = getElement('nav', ['menu-list']);
    const burgerMenu = getElement('button', ['menu-button']);
    ...
    clickBurgerMenu(burgerMenu, wrapper);

    menuNav.append(...menuItems);
    wrapper.append(menuNav);
    container.append(burgerMenu);
  }
}

const createHeaderSocialBlock = (social, wrapper) => {
  if (social) {
